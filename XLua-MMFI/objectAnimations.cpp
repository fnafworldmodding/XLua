...
int Animations::AnimationDirection(lua_State* L) {
	LPHO ho = GetHO(lua_touserdata(L, lua_upvalueindex(UV_OBJECT_LPHO)));
	tagRCOM* com = GetRCOM(ho);
	tagRA* ra = GetRA(ho);

	lua_pushnumber(L, ra->raAnimDir);
...
int Animations::SetAnimationDirection(lua_State* L) {
	LPHO ho = GetHO(lua_touserdata(L, lua_upvalueindex(UV_OBJECT_LPHO)));
	tagRCOM* com = GetRCOM(ho);
	tagRA* ra = GetRA(ho);

	ra->raAnimDir = lua_tointeger(L, PARAM1);
	com->rcChanged = 1;
